...
def update_score(l_score, r_score, player, score_board):
   ...
   score_board.clear()
   score_board.write('Player 1: {} -- Player 2: {}'.format(
...
def setup_game():
   ...
   screen.title('Pong Arcade Game (maybe buggy)')
...
def pong_game():
   ...
   while pong_game_alive:
       ...
       if ((ball.xcor() > 360) and
           (ball.xcor() < 370) and
           (ball.ycor() < r_paddle.ycor()+60) and
               (ball.ycor() > r_paddle.ycor()-60)):
           ball.setx(360)
           ball.dx *= -1

       if ((ball.xcor() < -360) and
               (ball.xcor() > -370) and
               (ball.ycor() < l_paddle.ycor()+60) and
               (ball.ycor() > l_paddle.ycor()-60)):
           ball.setx(-360)
           ball.dx *= -1
